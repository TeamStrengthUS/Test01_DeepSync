...
import TierManager from '../components/admin/TierManager.tsx';
...
const Overwatch: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'security' | 'logistics'>('security');
    ...
    <div className="space-y-12 p-2 text-left relative">
      <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-8 mb-12">
        <div className="flex items-center gap-6 text-left">
          ...
          <div>
            <h1 className="text-5xl font-black font-geist tracking-tighter mb-2 text-white">Overwatch: Command Center</h1>
            <p className="text-slate-400 font-medium flex items-center gap-2">
              <Terminal size={14} className="text-red-500" /> Administrative Hub for IHL Enforcement & Logistics.
            </p>
          </div>
        </div>
        
        {/* Tab Navigator */}
        <div className="flex p-1.5 bg-surface border border-white/5 rounded-2xl gap-1">
           <button 
            onClick={() => setActiveTab('security')}
            className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeTab === 'security' ? 'bg-red-500/10 text-red-500 border border-red-500/20 shadow-[0_0_20px_rgba(239,68,68,0.1)]' : 'text-slate-500 hover:text-slate-300'}`}
           >
             <ShieldAlert size={14} /> Security Shards
           </button>
           <button 
            onClick={() => setActiveTab('logistics')}
            className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeTab === 'logistics' ? 'bg-teal/10 text-teal border border-teal/20 shadow-[0_0_20px_rgba(45,212,191,0.1)]' : 'text-slate-500 hover:text-slate-300'}`}
           >
             <Package size={14} /> Logistics & Pricing
           </button>
        </div>
      </header>
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                ...
                <div 
                  key={i}
                  className={`p-8 ${stat.bg} ${stat.border} border rounded-[3rem] relative overflow-hidden group hover:scale-[1.02] transition-all duration-500 glass-card text-left`}
                >
                    ...
                    <AnimatePresence mode="wait">
                      {selectedNode ? (
                        ...
                        <div className="p-12 text-center border-2 border-dashed border-white/5 rounded-[2rem] text-slate-600">
                          <p className="text-xs font-black uppercase tracking-widest">Select Node for Inspection</p>
                        </div>
                      )}
                    </AnimatePresence>
                  </div>
