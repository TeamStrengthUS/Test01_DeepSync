...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (railwayToken && railwayServiceId) {
      ...
      // Fix: Corrected escaped quotes and assigned fetch response to use it later
      ...
      const deploymentId = railwayJson?.data?.deploymentCreate?.id;

      // Fix: Corrected escaped quotes and scope issues for Supabase update
      if (deploymentId) {
        await supabase
          .from("teammate_node")
          .update({ mesh_container_id: deploymentId, last_active: new Date().toISOString() })
          .eq("node_id", node_id)
          .eq("user_id", user_id);
      }
    }
